...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("clearDoneIdData").addEventListener("click", function () {
        clearDoneIdData();
    });
    ...
    //自動作成スール
    document.getElementById("createDatakeyId").addEventListener("click", function () {
    ...
    document.getElementById("createDatakeyBtn").addEventListener("click", function () {
    ...
    document.getElementById("confirmIdBtn").addEventListener("click", function () {
        ...
        if (isNaN(startVal) || isNaN(countVal) || countVal <= 0 || countVal > 100) {
...
function showDoneIdData() {
    ...
    if (doneIdResults.length === 0) {
        resultWindow.document.write("<p style='font-size:16px; font-family: sans-serif;'>完了IDはまだ保存されていません。</p>");
...
function saveData(data) {
    ...
    const idDisplay = (typeof currentId !== 'undefined' && currentId !== null) ? `${currentIdDisplay}` : "不明";
...
function showSavedDataInWindow(resultWindow, openedId = null, openedSubtab = null) {
    ...
    sortedIds.forEach((id, index) => {
        idTableRows += `
            <tr class="id-row" data-id="${id}" style="cursor: pointer;">
                <td style="text-align: center; border: 1px solid #ecf0f1; padding: 10px; font-size: 14px;">${index + 1}</td>
        ...
        const subTabs = ['Pass', 'Fail', 'SKIP', '非表示', '完了'].map(sub =>
            `<button class="sub-tab" data-subtab="${id}-${sub}">${sub} (${data[sub].length})</button>`
        ).join("");

        const subContents = ['Pass', 'Fail', 'SKIP', '非表示', '完了'].map(sub => {
            const colorMap = {
                'Pass': 'green',
                'Fail': 'red',
                'SKIP': '#FFBF00',
            ...
            const entries = data[sub].map(result => {
            ...
            return `<div id="${id}-${sub}" class="sub-content">${entries || '<p style="color: #888;">データなし</p>'}</div>`;
        }).join("");
    ...
    resultWindow.onload = () => {
        ...
        if (reloadBtn) {
            reloadBtn.addEventListener("click", () => {
                const activeIdElem = Array.from(resultWindow.document.querySelectorAll('.content'))
                ...
                if (activeIdElem) {
                    ...
                    if (activeSub) {
                        activeSubtab = activeSub.getAttribute('data-subtab').replace(`${activeId}-`, '');
                    }
                }
        ...
        const viewButtons = resultWindow.document.querySelectorAll('.view-result-btn');
        ...
        const deleteButtons = resultWindow.document.querySelectorAll(".delete-id-btn");
        ...
        deleteButtons.forEach(btn => {
            btn.addEventListener("click", function () {
                ...
                const sortedRemainingIds = Object.keys(groupedByIdAfterDelete).sort((a, b) => {
                    const parseId = id => {
                        const match = id.match(/(\d+)(?:_db2dc565)?/);
                        return match ? parseInt(match[1], 10) : Number.MAX_SAFE_INTEGER;
                    };
                ...
                const nextOpenedId = sortedRemainingIds[currentIndex + 1] || sortedRemainingIds[currentIndex - 1] || sortedRemainingIds[0] || null;
        ...
        if (openedId) {
            const button = resultWindow.document.querySelector(`.view-result-btn[data-id="${openedId}"]`);
            if (button) {
                button.click();
                if (openedSubtab) {
                    setTimeout(() => {
                        const subBtn = resultWindow.document.querySelector(`.sub-tab[data-subtab="${openedId}-${openedSubtab}"]`);
...
function showInputData() {
    ...
    let formattedResults = "<div style='font-family: Arial, sans-serif; display: flex; flex-direction: column; margin: 0 20px;'>";
    formattedResults += "<h3 style='color: #626567; font-weight: bold; text-align: center;'><<<<<<<<<<データの取得を表示>>>>>>>>></h3>";

    inputResults.forEach(result => {
        formattedResults += "<div style='display: flex; border: 1px solid #000; padding: 10px;'>";
        ...
        if (typeof result === "string") {
            ...
            formattedResults += `<div style='color: #1a5276; font-weight: bold; width: 80px; text-align: left;'>${result.id || "?"} :</div>`;
            formattedResults += `<div style='color: #1a5276; font-weight: bold; padding-left: 10px'>${result.value || ""}</div>`;
        }
...
async function epoca_AutoCheck(epocaData, currentIdDisplay) {
    ...
    for (let i = 0; i < epocaData.length; i++) {
        ...
        if (i > 0 && epocaData[i].blade != "THANKPAGE") {
            ...
            if (currentNumber.value - nowQ === 1) {
                ...
                if (currentNumber.value!= null && nowQ > currentNumber.value){
                    saveData(`⛔ Fail: Warning : ${epocaData[i-1].blade}の進捗があります。`);
                    return;
                }
                ...
                if (currentNumber.value!= null && currentNumber.value - nowQ === 1) {
                    ...
                    saveData(`🏁 SKIP: Blade ${epocaData[i].blade} はスキップされています。`);
                    nowQ++;
                    continue;
                }
            }
        ...
        const typeList = (autoData?.type || "").split(",").map(type => type.trim());
        ...
        for (const type of typeList) {
            switch (type) {
                ...
                case "NO":
                    ...
                    // Num Check
                    const numCheckResult = await sendToContent("numCheckAutoTest", { jsonData: autoData });
                    let allPass = true;
                    ...
                    if (!isAlertBox.status) {
                        ...
                        saveData(`✅ Pass: Blade ${autoData.blade} 初期ランダムチェックエラーなし`);
                        break; 
                    }
                    ...
                    if (autoData?.saikei) {
                        ...
                        if (Array.isArray(saikeiResult.data) && saikeiResult.data.some(item => item.status === false)) {
                            await new Promise(resolve => setTimeout(resolve, 500));
                            await sendToContent("nextBtnClick");
                    ...
                    if (autoData?.gokei) {
                        ...
                        const gokeiResult = await sendToContent("goKeiSaikeiCheck", { jsonData: autoData });
                        ...
                        if (!gokeiResult.data.status) {
                            ...
                            isAlertBox = await sendToContent("alertCheck");
                            if (isAlertBox.status) {
                                saveData(`✅ Pass: Blade ${autoData.blade} 合計アウトのチェック`);
                            } else {
                                saveData(`❌ Fail: Blade ${autoData.blade} 合計アウトのチェック`);
                            }
                    ...
                    if (autoData?.gokei100) {
                        await sendToContent("rndStep10Click");
                        const gokei100Result = await sendToContent("goKei100Check", { jsonData: autoData });
                        ...
                        if (!gokei100Result.data.status) {
                            ...
                            isAlertBox = await sendToContent("alertCheck");
                            if (isAlertBox.status) {
                                saveData(`✅ Pass: Blade ${autoData.blade} 合計が100%となるのアウトのチェック`);
                            } else {
                                saveData(`❌ Fail: Blade ${autoData.blade} 合計が100%となるのアウトのチェック`);
                            }

                            await new Promise(resolve => setTimeout(resolve, 500));
                            await sendToContent("autoInputByGokei100", { jsonData: autoData });
                            noEntry = await sendToContent("getNoData");
                            await sendToContent("nextBtnClick");
                        }
                    }
                    ...
                    if (autoData?.totalValue) {
                        await sendToContent("rndStep10Click");
                        const totalValueResult = await sendToContent("totalValueCheck", { jsonData: autoData });
                        ...
                        if (!totalValueResult.data.status) {
                            ...
                            isAlertBox = await sendToContent("alertCheck");
                            if (isAlertBox.status) {
                                saveData(`✅ Pass: Blade ${autoData.blade} 各項目の合計が${autoData.totalValue}となるように入力のチェック`);
                            } else {
                                saveData(`❌ Fail: Blade ${autoData.blade} 各項目の合計が${autoData.totalValue}となるように入力のチェック`);
                            }
                    ...
                    if (autoData?.tgokeiSpecial) {
                        await sendToContent("autoInputFromTotalColumn", { jsonData: autoData });
                        ...
                        isAlertBox = await sendToContent("alertCheck");
                        if (isAlertBox.status) {
                            saveData(`✅ Pass: Blade ${autoData.blade} 列合計が再掲となるように入力のチェック`);
                        } else {
                            saveData(`❌ Fail: Blade ${autoData.blade} 列合計が再掲となるように入力のチェック`);
                        }
                    }
                    ...
                    if (autoData?.ygokeiSpecial) {
                        ...
                        isAlertBox = await sendToContent("alertCheck");
                        if (isAlertBox.status) {
                            saveData(`✅ Pass: Blade ${autoData.blade} 横合計が再掲となるように入力のチェック`);
                        } else {
                            saveData(`❌ Fail: Blade ${autoData.blade} 横合計が再掲となるように入力のチェック`);
                        }
                    }
                    ...
                    if (autoData?.input) {
                        await sendToContent("nextBtnClick");
                        await new Promise(resolve => setTimeout(resolve, 1000));
                        await sendToContent("autoInputDepressionCheck", { jsonData: autoData });
                        await new Promise(resolve => setTimeout(resolve, 1000));
                        await sendToContent("nextBtnClick");
                        await new Promise(resolve => setTimeout(resolve, 500));
                        isAlertBox = await sendToContent("alertCheck");
                        if (isAlertBox.status == false){
                            saveData(`✅ Pass: Blade ${autoData.blade} 再掲●入力値＝入力値計`);
                        } else {
                            saveData(`❌ Fail: Blade ${autoData.blade} 再掲●入力値＝入力値計`);
                        }
                    } 
                    ...
                    if (isAlertBox.status) {
                        ...
                        saveData(`✅ Pass: Blade ${autoData.blade} アラートの条件に従って入力値のチェック`);
                        await new Promise(resolve => setTimeout(resolve, 500));
                        await sendToContent("nextBtnClick");
                    } 
                ...
                case "MA":
                    ...
                    if (errBoxMaCheck.status) {
                        ...
                        if (errBoxMaCheck.status) {
                            await sendToContent("nextBtnClick");
                            await new Promise(resolve => setTimeout(resolve, 1000));
                            const maxCheckBoxResult = await sendToContent("autoCheckMaxErrorCheckbox");
                            saveData(`✅ Pass: Blade ${autoData.blade} Max CheckBox : ${maxCheckBoxResult.maxcheckbox}`);
                        } else {
                ...
                default:
                    ...
                    saveData(`⚠ 未対応のtype (${autoData?.type}) - ID ${currentIdDisplay}`);
                    break;
            }
        }
...
async function ro_Data(currentIdDisplay) {
    ...
    while (tsgResult.status === false && doneResult.status === false && loopTemp < 200) {
        if (isStopped) return "停止中";
        while (isPaused) {
        ...
        let bladeItem = {
            blade: Array.isArray(bladeResult?.blade) ? bladeResult.blade.join(", ") : bladeResult?.blade || "",
            type: Array.isArray(bladeResult?.type) ? bladeResult.type.join(", ") : bladeResult?.type || ""
        };
            ...
            switch (type) {
                ...
                case "MASA":
                    ...
                    while ( flag == 0) {
                        await sendToContent("rndSaBtnClick");
                        await new Promise(resolve => setTimeout(resolve, 1000));
                        const matchCheck = await sendToContent("checkRadioAndCheckboxSameRow");
                    ...
                    if (errBoxMaSaCheck.status) {
                        ...
                        if (alertIsHasSonota.includes("他")) {
                            const isCheckboxSameValueChecked = await sendToContent("isCheckboxSameValueChecked");
                    ...
                    try {
                        ...
                        if (errBoxCheck.status) {
                            ...
                            if (alertText.includes("%")|| alertText.includes("％")) {
                                console.log("合計: 100%");
                                bladeItem.gokei100 = "=";
                                await sendToContent("autoInputByGokei100", { jsonData: bladeResult });
                                ...
                                if (alertText.includes("上")) {
                                    ...
                                    bladeItem.gokei = ">=";
                                    bladeItem.saikei = ">=";
                                    await sendToContent("autoInputByGokei", { jsonData: bladeResult });
                                } else if (alertText.includes("下")) {
                                    let gokeiResult = await sendToContent("autoInputByGokei", { jsonData: bladeResult });
                                    const isEmpty = !gokeiResult || !Array.isArray(gokeiResult.data) || gokeiResult.data.length === 0;

                                    if (isEmpty) {
                                        const fallbackResult = await sendToContent("autoInputFromTotalRow");
                                    ...
                                    console.log("合計: 横");
                                    await sendToContent("autoInputByGokeiY", { jsonData: bladeResult });
                                    await new Promise(resolve => setTimeout(resolve, 1000));
                                    await sendToContent("nextBtnClick");
                            ...
                            } else if (alertText.includes("以") && (bladeResult.hasT || bladeResult.hasY)) {
                                if (alertText.includes("上")) {
                                    console.log("再掲: 以上");
                                    bladeItem.saikei = "<=";
                                    await sendToContent("autoInputBySaikei", { jsonData: bladeResult });
                                } else if (alertText.includes("下")) {
                                    console.log("再掲: 以下");
                                    bladeItem.saikei = ">=";
                                    await sendToContent("autoInputBySaikei", { jsonData: bladeResult });
    ...
    const mergedBladeMapList = bladeMapList.reduce((acc, item) => {
        const existing = acc.find(el => el.blade === item.blade && el.type === item.type);
    ...
    if (IsCreateJsonCheck.status === true){
        ...
        const fileName = `EPOCA_ID${currentIdDisplay}_TEST_${new Date().toISOString().replace(/[:.]/g, "-")}.json`;
        a.href = url;
        a.download = fileName;
...
async function createDatakey() {
    ...
    console.log("Post-Key Data: ",dataQ.data);
}
